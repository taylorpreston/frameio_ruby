module Frameio
  class ReviewLinks < APIResource
    ...
    # @param frameio_token [FrameioAuthToken] Auth token retrieved from Frame.io
    # @param project_id [String] the Frame.io project id.
    # @param project_id [Object] the request body.
    def self.create(frameio_token:, project_id:, body:)
      request(:get, "#{BASE_URL}/projects/#{project_id}/review_links", authorize_bearer_header(frameio_token), body:)
    end
    ...
    def self.update(frameio_token:, review_link_id:, body:)
      request(:get, "#{self.resource_url}/#{review_link_id}", authorize_bearer_header(frameio_token), body:)
    end
