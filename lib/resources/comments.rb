module Frameio
  class Comments < APIResource
    ...
    def self.list_replies(frameio_token:, comment_id:)
      request(:get, "#{self.resource_url}/#{comment_id}/replies", authorize_bearer_header(frameio_token))
    end

    # @param frameio_token [FrameioAuthToken] Auth token retrieved from Frame.io
    def self.list(frameio_token:, asset_id:)
      request(:get, "#{BASE_URL}/assets/#{asset_id}/comments", authorize_bearer_header(frameio_token))
    end
    
    # @param frameio_token [FrameioAuthToken] Auth token retrieved from Frame.io
    # @param comment_id [String] The Frame.io user's account ID
    def self.find(frameio_token:, comment_id:)
      request(:get, "#{self.resource_url}/#{comment_id}", authorize_bearer_header(frameio_token))
    end
    ...
    def self.create_reply(frameio_token:, comment_id:, body:)
      request(:post, "#{self.resource_url}/#{comment_id}/replies", authorize_bearer_header(frameio_token), body)
    end
    ...
    def self.create(frameio_token:, asset_id:, body:)
      request(:post, "#{BASE_URL}/assets/#{asset_id}/comments", authorize_bearer_header(frameio_token), body)
    end
    ...
    def self.destroy(frameio_token:, comment_id:)
      request(:delete, "#{self.resource_url}/#{comment_id}", authorize_bearer_header(frameio_token))
    end
